...
import { IRegister, register, sendEmailCode } from '@/apis/register'
...
const { errorMessage: unameError, value: username } = useField<string>('username', {}, { label: '用户名' })
const { errorMessage: pwdError, value: password } = useField<string>('password', {}, { label: '密码' })
const { errorMessage: pwd_confirmError, value: password_confirmation } = useField<string>('password_confirmation', {}, { label: '确认密码' })
...
const sendEmailText = ref<string | number>('发送验证码')
const sendEamilBtn = ref<HTMLButtonElement>()
...
const sendEmail = async () => {
  if (!emailError.value && email.value) {
    ...
    console.log(result)
    changeSendState(sendEamilBtn.value!)
    intervalId.value = setInterval(() => {
      sendEmailText.value <= 0 ? changeSendState(sendEamilBtn.value!) : (sendEmailText.value as number)--
    }, 1000)
  }
}
</script>
    ...
    <section class="flex flex-col md:flex-row h-screen items-center">
      ...
      >
        <div class="w-full h-100">
          <h1 class="text-xl md:text-2xl font-bold leading-tight mt-12 text-center">注册账号</h1>
            ...
            <div class="mt-4">
              ...
              <div class="flex items-center">
                <input
                  ...
                  placeholder="请输入验证码"
                  minlength="6"
                  class="px-4 py-3 w-full rounded-lg bg-gray-200 border focus:border-blue-500 focus:bg-white focus:outline-none"
                  required
                />
              </div>
          ...
          <div class="mt-2 gap-4 flex items-center">
            <router-link to="/login" class="text-blue-500 text-right hover:text-blue-700 font-semibold">前往登录</router-link>
