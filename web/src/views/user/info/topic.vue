...
const fetch = async () => {
  ...
  const { result } =
    (await fetchArticles({
      where: { author: user.profile?._id, title: { $regex: search.value ?? '' }, status: [Status.PRIVATE, Status.ISSUE] },
      sort: '-createdAt',
...
<template>
  <div class="topic">
    ...
    <template v-if="articles?.length && user.isLogin">
      <div @click="$router.push(`/post/${article._id}`)" class="item" v-for="article of articles" :key="article._id">
        ...
        <div class="action">
          ...
          <div>
            <Badge color="#e67e22" v-if="article.status === Status.PRIVATE">审核中</Badge>
            <Badge color="#2ecc71" v-else-if="article.status === Status.ISSUE">已发布</Badge>
          </div>
        </div>
      </div>
    </template>
...
.topic {
  ...
  .item {
    ...
    .content {
      ...
      .category {
        @apply md:flex items-center gap-4 text-xs hidden;
        .cate {
          span {
