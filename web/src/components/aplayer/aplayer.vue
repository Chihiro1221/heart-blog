...
import { MetingMusic, Player } from './player';
...
onMounted(() => {
  nextTick(() => {
    Player.getSongSheet(props.songServer, props.songType, props.songId).then((res) => {
      let audioList = res.data.map((value: MetingMusic) => new Audio(value.author, value.title, value.url, value.pic, value.lrc));
